...
class ManualEnrichmentAreaTest extends ApplicationManagedAreaTestBase {
  ...
  describe("upsert method") {
    it("Should publish full snapshot of records that are not defined in silver and publish areas on each call") {
      ...
      val area              = new ManualEnrichmentArea(areaPath, timestampProvider)
      ...
      val secondExpected = firstExpected.withColumn(UndefinedText.UPDATED_AT, lit(secondTimestamp)).as[UndefinedText]
    ...
    it("Should handle multiline text") {
      import spark.implicits._
      val area = new ManualEnrichmentArea(areaPath, testTimestampProvider)
      val silverSnapshot = spark.createDataset(
        Seq(
          DefinedText(
            "a\na",
